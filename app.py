...
# 【追加】通貨ペア・銘柄のリストを作成し、プルダウンで選べるようにする
symbols = {
...
selected_name = st.sidebar.selectbox("銘柄を選択", list(symbols.keys()))
symbol = symbols[selected_name] # 選んだ名前に対応する裏側のコード（ティッカー）を取得
...
# --- データ取得 ---
# 仮想通貨などは週末も動いているため、少し多めにデータを取得します
...
if not df.empty:
    ...
    for i in range(len(df)):
        if df['ADX'].iloc[i] >= adx_threshold:
            if df['Close'].iloc[i] > df['EMA'].iloc[i]:
                raw_colors.append(0) # 上昇
            else:
                raw_colors.append(2) # 下降
        else:
            raw_colors.append(1) # レンジ
            
    df['RawColor'] = raw_colors

    # --- ノイズフィルターの適用 ---
    filtered_colors = [1] * len(df)
    ...
    st.subheader(f"💡 現在のMarketPhase ({selected_name})")
    if latest_color == 0:
    ...
    st.warning("データの取得に失敗しました。週末や市場の休場時間の可能性があります。")
